...
export default class ConfigManager {
    ...
    private openAIConfig: OpenAIConfigType = {
        ...
        summaryModel: config.openAI.summaryModel || process.env.OPENAI_SUMMARY_MODEL || "gpt-4o-mini",
        prompt: config.openAI.prompt || process.env.OPENAI_PROMPT || "You are a nice assistant in a discord server",
    ...
    private discordConfig: DiscordConfigType = {
        ...
        maxHistory: config.discord.maxHistory || Number(process.env.DISCORD_MAX_HISTORY) || 10,
    ...
    private serpConfig: SerpConfigType = {
        ...
        google_domain: config.serp.google_domain || process.env.SERP_GOOGLE_DOMAIN || ""
    };

    private braveSearchConfig: BraveSearchConfigType = {
        active: config.braveSearch.active || process.env.BRAVE_SEARCH_ACTIVE || false,
    ...
    private coinConfig: CoinConfigType = {
        ...
        defaultAsset: config.coin.defaultAsset || process.env.COIN_DEFAULT_ASSET || "USD"
    };
