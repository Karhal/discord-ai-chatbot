...
class AiCompletionHandler {
  ...
  createMessagesArrayFromHistory(messagesChannelHistory:any) {
    ...
    let messages: { role: string; content: string; channelId: any; }[] = [];
    messagesChannelHistory.reverse().forEach((msg: { content: string; author: { bot: any; username: any; }; createdAt: any; channelId: any; }) => {
        if(msg.content !== '') {
